...
const FIXTURE: &str = include_str!("../../../fixtures/protocol-v1/enrolment/pistis-enrolment-v1-cross-project-001.json");
...
fn binding_schema(bytes: &[u8]) -> Result<(), ()> {
    let Value::Map(fields) = from_slice(bytes).map_err(|_| ())? else { return Err(()) };
    if fields.len() != 16 || fields.keys().copied().collect::<Vec<_>>() != (0..16).collect::<Vec<_>>() {
        return Err(());
    }
...
fn cross_project_fixture_is_byte_exact_and_strict() {
    ...
    let Value::Map(mut changed) = from_slice(&binding).unwrap() else { unreachable!() };
    ...
    assert_eq!(binding_decoded.signature().as_slice(), hex("binding_signature_raw_low_s_hex"));
    assert_eq!(lookup_decoded.signature().as_slice(), hex("lookup_signature_raw_low_s_hex"));
    let protected = hex("protected_headers_hex");
    let Value::Array(binding_parts) = from_slice(&binding_envelope).unwrap() else { unreachable!() };
    assert_eq!(binding_parts[0], Value::Bytes(protected.clone()));
    let Value::Array(lookup_parts) = from_slice(&lookup_envelope).unwrap() else { unreachable!() };
    assert_eq!(lookup_parts[0], Value::Bytes(protected.clone()));
    assert_eq!(signing_input(&binding, key_id).unwrap(), hex("binding_sig_structure_hex"));
    assert_eq!(signing_input(lookup_decoded.payload(), key_id).unwrap(), hex("lookup_sig_structure_hex"));
    ...
    assert_eq!(sha256(&commit).as_bytes().as_slice(), hex("commit_digest_sha256_hex"));

    let Value::Array(commit_fields) = from_slice(&commit).unwrap() else { unreachable!() };
    assert_eq!(commit_fields.len(), 11);
    assert_eq!(commit_fields[0], Value::Text("pistis.commit-enrolment.v1".into()));
    ...
    let Value::Map(mut lookup_fields) = from_slice(&lookup).unwrap() else { unreachable!() };
    assert_eq!(lookup_fields.len(), 5);
    assert_eq!(lookup_fields.keys().copied().collect::<Vec<_>>(), (0..5).collect::<Vec<_>>());
    assert_eq!(lookup_fields.remove(&0), Some(Value::Text("pistis.enrolment-receipt-lookup.v1".into())));
    ...
    assert_eq!(lookup_fields.remove(&3), Some(Value::Bytes(hex("commit_digest_sha256_hex"))));
    assert_eq!(lookup_fields.remove(&4), Some(Value::Text("prosopikon:pistis:enrolment".into())));
    let Value::Map(mut extra_lookup) = from_slice(&lookup).unwrap() else { unreachable!() };
