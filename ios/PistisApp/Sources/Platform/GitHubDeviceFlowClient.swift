...
struct GitHubDeviceFlowClient: Sendable {
  ...
  private func validJSONContentType(_ response: GitHubHTTPResponse) -> Bool {
    ...
    guard components.count == 1 || components.count == 2,
      trimHTTPWhitespace(components[0]).lowercased() == "application/json"
    else {
      return false
    }
    ...
    return parameter.count == 2
      && trimHTTPWhitespace(parameter[0]).lowercased() == "charset"
      && trimHTTPWhitespace(parameter[1]).lowercased() == "utf-8"
  }
    ...
    for key in ["error_description", "error_uri"] {
      if let value = object[key] {
        guard case .string(let text) = value,
          isBoundedASCIIText(text, range: 1...1_024)
        else {
          throw GitHubDeviceFlowFailure.malformedResponse
        }
      }
  ...
  private func isBoundedASCIIText(
    _ value: String,
    range: ClosedRange<Int>
  ) -> Bool {
    range.contains(value.utf8.count)
  ...
  private func trimHTTPWhitespace(_ value: Substring) -> Substring {
