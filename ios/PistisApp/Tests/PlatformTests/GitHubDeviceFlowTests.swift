...
final class GitHubDeviceFlowTests: XCTestCase {
  ...
  func testProviderErrorDescriptionMayContainBoundedSpaces() async throws {
    let transport = ScriptedGitHubTransport([
      success(
        url: endpoint("https://github.com/login/oauth/access_token"),
        json:
          #"{"error":"authorization_pending","error_description":"The authorization request is still pending.","error_uri":"https://docs.github.com/apps/oauth"}"#
      )
    ])
    let result = try await makeClient(transport: transport).poll(
      capability: TransientGitHubSecret(bytes: Data(repeating: 0x61, count: 40))
    )
    guard case .pending = result else {
      return XCTFail("valid provider description changed pending semantics")
    }
  }

  func testRetryAfterOnOtherServerErrorCannotChangeBackoff() async throws {
    let tokenURL = endpoint("https://github.com/login/oauth/access_token")
    let transport = ScriptedGitHubTransport([
      success(
        url: endpoint("https://github.com/login/device/code"),
      ...
      success(url: tokenURL, json: tokenJSON()),
      success(
        url: endpoint("https://api.github.com/user"),
        json: #"{"id":3848500,"login":"sagrudd"}"#
      ),
    ...
    XCTAssertEqual(
      sleeps,
      [2, 5].map { UInt64($0) * 1_000_000_000 }
    )
  }

  func testMalformedContentTypeSpacingFailsClosed() async throws {
    let transport = ScriptedGitHubTransport([
      response(
        url: endpoint("https://github.com/login/device/code"),
        status: 200,
        headers: ["Content-Type": "application / json"],
        body: Data(deviceAuthorizationJSON().utf8)
      )
    ])
    do {
      _ = try await makeClient(transport: transport).requestDeviceAuthorization()
      XCTFail("malformed media type was accepted")
    } catch {
      XCTAssertEqual(error as? GitHubDeviceFlowFailure, .malformedResponse)
    }
  }

  func testCancellationRejectsLateDeviceCodeResponse() async throws {
    let transport = GatedGitHubTransport(
      [
        success(
          url: endpoint("https://github.com/login/device/code"),
          json: deviceAuthorizationJSON()
    ...
    do {
      ...
      XCTFail("late device-code response revived a cancelled attempt")
    } catch {
      XCTAssertEqual(error as? GitHubDeviceFlowFailure, .cancelled)
    }
    let phase = await coordinator.phase
    XCTAssertEqual(phase, .cancelled)
  }

  func testLateCancelledGenerationCannotClearReplacementAttempt() async throws {
    let firstResponse = success(
      url: endpoint("https://github.com/login/device/code"),
      json: deviceAuthorizationJSON()
    )
    let transport = GatedGitHubTransport(
    ...
    let staleStart = Task { try await coordinator.start() }
    await transport.waitUntilBlocked()
    await coordinator.cancel()
    let replacementPrompt = try await coordinator.start()
    await transport.release()
    ...
    let phase = await coordinator.phase
    XCTAssertEqual(phase, .awaitingBrowser(replacementPrompt))
  }

  func testCancellationRejectsLateAuthenticatedUserResponse() async throws {
    let transport = GatedGitHubTransport(
      [
        success(
          url: endpoint("https://github.com/login/device/code"),
    ...
    _ = try await coordinator.start()
    try await coordinator.systemBrowserDidOpen()
    let verification = Task { try await coordinator.resumeVerification() }
    await transport.waitUntilBlocked()
    await coordinator.cancel()
    await transport.release()
    ...
    do {
      ...
      XCTFail("late /user response revived a cancelled attempt")
    } catch {
      XCTAssertEqual(error as? GitHubDeviceFlowFailure, .cancelled)
    }
    let phase = await coordinator.phase
  ...
  func testMonotonicRegressionAcrossPollFailsClosed() async throws {
    let transport = ScriptedGitHubTransport([
      success(
        url: endpoint("https://github.com/login/device/code"),
        json: deviceAuthorizationJSON(expires: 120, interval: 2)
    ...
    let clock = AdvancingGitHubClock(regressOnNowCall: 5)
    let coordinator = try GitHubDeviceFlowCoordinator(
      client: makeClient(transport: transport),
      clock: clock,
      attemptIDGenerator: FixedGitHubAttemptIDGenerator()
    ...
    do {
      _ = try await coordinator.resumeVerification()
      XCTFail("regressing monotonic clock was accepted")
    } catch {
      XCTAssertEqual(error as? GitHubDeviceFlowFailure, .attemptExpired)
    }
  }

  func testMonotonicRegressionAcrossAuthenticatedUserFailsClosed() async throws {
    let transport = ScriptedGitHubTransport([
      success(
        url: endpoint("https://github.com/login/device/code"),
        json: deviceAuthorizationJSON(expires: 120, interval: 2)
    ...
    let clock = AdvancingGitHubClock(regressOnNowCall: 7)
    let coordinator = try GitHubDeviceFlowCoordinator(
      client: makeClient(transport: transport),
      clock: clock,
      attemptIDGenerator: FixedGitHubAttemptIDGenerator()
    ...
    do {
      _ = try await coordinator.resumeVerification()
      XCTFail("regressing monotonic clock after /user was accepted")
    } catch {
      XCTAssertEqual(error as? GitHubDeviceFlowFailure, .attemptExpired)
    }
  }
...
private actor GatedGitHubTransport: GitHubDeviceFlowTransport {
  private var responses: [GitHubHTTPResponse]
  ...
  init(
    _ responses: [GitHubHTTPResponse],
    blockedRequestIndex: Int
  ) {
    self.responses = responses
  ...
  func perform(_ request: URLRequest) async throws -> GitHubHTTPResponse {
    ...
    requestIndex += 1
    if currentIndex == blockedRequestIndex {
      blocked = true
...
private actor AdvancingGitHubClock: GitHubDeviceFlowClock {
  ...
  private let regressOnNowCall: Int?
  private(set) var sleeps: [UInt64] = []

  init(regressOnNowCall: Int? = nil) {
    self.regressOnNowCall = regressOnNowCall
  }

  func nowNanoseconds() async throws -> UInt64 {
    nowCalls += 1
    if nowCalls == regressOnNowCall {
      now -= 1
    }
    return now
  }
